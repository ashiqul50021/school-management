...
<section class="content">
 ...
 <form method="post" action="{{ route('update.fee.amount',$feeCategoryAmount[0]->fee_category_id) }}">
	 	@csrf
					  <div class="row">
						<div class="col-12">
						<div class="add_item">
	...
	<div class="controls">
	 <select name="fee_category_id" required="" class="form-control">
		...
		<option value="{{ $category->id }}" {{ ($feeCategoryAmount['0']->fee_category_id == $category->id)? "selected":"" }}>{{ $category->name }}</option>
		@endforeach	 
		</select>
	 </div>
          </div> <!-- // end form group -->
  ...
  @foreach($feeCategoryAmount as $edit)
